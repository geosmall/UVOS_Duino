...
#ifndef W25QXX_QSPI
    w25qxx->hal.gpio_write(w25qxx->cs_port, w25qxx->cs_pin, W25QXX_HAL_RESET);
#endif
}
...
static inline void cs_off(W25QXX_HandleTypeDef *w25qxx) {
    w25qxx->hal.gpio_write(w25qxx->cs_port, w25qxx->cs_pin, W25QXX_HAL_SET);
}
...
W25QXX_result_t w25qxx_transmit(W25QXX_HandleTypeDef *w25qxx, uint8_t *buf, uint32_t len) {
    ...
    if (w25qxx->hal.spi_transmit(w25qxx->spiHandle, buf, len, W25QXX_HAL_MAX_DELAY) == W25QXX_HAL_OK) {
        ret = W25QXX_Ok;
    }
    return ret;
}
...
W25QXX_result_t w25qxx_receive(W25QXX_HandleTypeDef *w25qxx, uint8_t *buf, uint32_t len) {
    W25QXX_result_t ret = W25QXX_Err;
    if (w25qxx->hal.spi_receive(w25qxx->spiHandle, buf, len, W25QXX_HAL_MAX_DELAY) == W25QXX_HAL_OK) {
        ret = W25QXX_Ok;
    }
    return ret;
}
...
W25QXX_result_t w25qxx_wait_for_ready(W25QXX_HandleTypeDef *w25qxx, uint32_t timeout) {
    ...
    uint32_t begin = w25qxx->hal.get_tick();
    uint32_t now = w25qxx->hal.get_tick();
    // Wait until the busy flags disappear.
    while ((now - begin <= timeout) && (w25qxx_get_status(w25qxx) & 0x01)) {
        now = w25qxx->hal.get_tick();
    }
...
W25QXX_result_t w25qxx_read(W25QXX_HandleTypeDef *w25qxx, uint32_t address, uint8_t *buf, uint32_t len) {
    ...
    if (w25qxx_wait_for_ready(w25qxx, W25QXX_HAL_MAX_DELAY) != W25QXX_Ok) {
        return W25QXX_Err;
    }
