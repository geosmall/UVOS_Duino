                                  ...
                                  void* callback_context)
{
    ...
    uvs_dma_invalidate_cache_for_buffer(buff, size);
...
pin_alt pins_none = {{UVS_GPIOX, 0}, 255};
...
pin_alt uart4_pins_tx[] = {{{UVS_GPIOB, 9}, GPIO_AF8_UART4},
                           ...
                           {{UVS_GPIOA, 0}, GPIO_AF8_UART4}};
pin_alt uart4_pins_rx[] = {{{UVS_GPIOB, 8}, GPIO_AF8_UART4},
...
UartHandler::Result
checkPinMatch(GPIO_InitTypeDef* init, uvs_gpio_pin pin, int p_num)
{
    for(int i = 0; i < 3; i++)
    {
        if(uvs_pin_cmp(&pins_periphs[p_num][i].pin, &pins_none.pin))
        {
            /* skip */
        }

        else if(uvs_pin_cmp(&pins_periphs[p_num][i].pin, &pin))
        {
...
UartHandler::Result UartHandler::Impl::InitPins()
{
    ...
    if(config_.pin_config.tx.port != UVS_GPIOX)
    {
        ...
        //setup tx pin
        GPIO_TypeDef* port  = uvs_hal_map_get_port(&config_.pin_config.tx);
        GPIO_InitStruct.Pin = uvs_hal_map_get_pin(&config_.pin_config.tx);
        HAL_GPIO_Init(port, &GPIO_InitStruct);
    }

    if(config_.pin_config.rx.port != UVS_GPIOX)
    {
        ...
        //setup rx pin
        GPIO_TypeDef* port  = uvs_hal_map_get_port(&config_.pin_config.rx);
        GPIO_InitStruct.Pin = uvs_hal_map_get_pin(&config_.pin_config.rx);
        HAL_GPIO_Init(port, &GPIO_InitStruct);
    }
...
{
    GPIO_TypeDef* port = uvs_hal_map_get_port(&config_.pin_config.tx);
    uint16_t      pin  = uvs_hal_map_get_pin(&config_.pin_config.tx);
    HAL_GPIO_DeInit(port, pin);

    port = uvs_hal_map_get_port(&config_.pin_config.rx);
    pin  = uvs_hal_map_get_pin(&config_.pin_config.rx);
    HAL_GPIO_DeInit(port, pin);
...
void HAL_UART_MspInit(UART_HandleTypeDef* uartHandle)
{
    ...
    uvs_hal_map_gpio_clk_enable(handle->config_.pin_config.rx.port);
    uvs_hal_map_gpio_clk_enable(handle->config_.pin_config.tx.port);
...
extern "C" void uvs_uart_global_init()
{
...
static void UART_CheckRxListener(UartHandler::Impl* handle)
{
    ...
    if(handle->circular_rx_callback_ && pos != old_pos)
    {
        if(pos > old_pos)
        {
            ...
            {
                /** Cache Invalidate */
                uvs_dma_invalidate_cache_for_buffer(&buffer[old_pos],
                                                    pos - old_pos);
        ...
        else
        {
            ...
            if(handle->circular_rx_callback_)
            {
                ...
                uvs_dma_invalidate_cache_for_buffer(&buffer[old_pos], rx_size);
                handle->circular_rx_callback_(&buffer[old_pos],
                                              rx_size,
                                              handle->circular_rx_context_,
                                              UartHandler::Result::OK);
                ...
                uvs_dma_invalidate_cache_for_buffer(&buffer[0], pos);
...
extern "C" void HAL_UART_ErrorCallback(UART_HandleTypeDef* huart)
{
    ...
    if (handle->listener_mode_)
    {
        ...
        /** cache maintanence to allow memory from cache-able regions  */
        uvs_dma_invalidate_cache_for_buffer(handle->circular_rx_buff_, handle->circular_rx_total_size_);
